...
--  Octubre 25, 2018
--  MCV: Buro Califica. Modificación, se crea copia, se agrega la variable 'CalificacionCarteraBanxico' .


USE DBMYMWORK;

DROP TABLE IF EXISTS JEA_MCV_BURO_CAL_PM;

CREATE TABLE IF NOT EXISTS JEA_MCV_BURO_CAL_PM 
AS 
SELECT * FROM JEA_MCV_CALIFICA;

ALTER TABLE JEA_MCV_BURO_CAL_PM ADD INDEX LLAVE(RFC);
ALTER TABLE JEA_MCV_BURO_CAL_PM ADD INDEX LLAVE1(FECHACONSULTA);

--  AGREGAMOS LA INFORMACION DEL FOLIO 

ALTER TABLE JEA_MCV_BURO_CAL_PM ADD COLUMN CalificacionCarteraBanxico VARCHAR(503) AFTER RFC;

UPDATE JEA_MCV_BURO_CAL_PM  T1
LEFT JOIN (SELECT DISTINCT RFC,FOLIO, FECHACONSULTA,CalificacionCarteraBanxico
FROM JEA_MCV_CONSULTAS) T3
ON T3.RFC = T1.RFC AND T3.FOLIO = T1.FOLIO
SET T1.CalificacionCarteraBanxico = T3.CalificacionCarteraBanxico;

SELECT * FROM JEA_MCV_BURO_CAL_PM LIMIT 150;

--  CAMBIAMOS '--' POR NULO DE LA TABLA
UPDATE JEA_MCV_BURO_CAL_PM
SET 
