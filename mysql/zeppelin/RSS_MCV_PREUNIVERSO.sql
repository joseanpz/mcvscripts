...
CREATE TABLE IF NOT EXISTS RSS_MCV_PREUNIVERSO 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' 

LOCATION 's3://boi-banregio/datalake/data/InteligenciaRiesgos/M&M/MCV/RSS_MCV_PREUNIVERSO' AS
...
INNER JOIN (SELECT distinct llave_universal from RSS_MCV_CATALOGO_LIN_REEST) C
ON A.LLAVE_UNIVERSAL_20 = C.llave_universal 

INNER JOIN (SELECT  distinct LLAVE_UNIVERSAL_20 from RSS_MCV_CATALOGO_LIN_NAC_VENC ) D
ON A.LLAVE_UNIVERSAL_20 = D.LLAVE_UNIVERSAL_20

/*Para tipo de creditos */
--INNER JOIN (SELECT tipo_de_credito from RSS_MCV_CATALOGO_REVOLVENTE) E
--ON A.tipo_de_credito = E.tipo_de_credito 

--INNER JOIN (SELECT tipo_de_credito from RSS_MCV_CATALOGO_NO_REVOLVENTE ) F
