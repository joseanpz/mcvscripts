%hive
DROP TABLE IF EXISTS RSS_MCV_UNIVERSO_NEGOCIOSA;

CREATE TABLE IF NOT EXISTS
RSS_MCV_UNIVERSO_NEGOCIOSA 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' 
LOCATION 's3://boi-banregio/datalake/data/InteligenciaRiesgos/M&M/MCV/RSS/RSS_MCV_UNIVERSO_NEGOCIOSA' AS
SELECT
  A.RFC,  
...
LEFT JOIN JEA_MCV_UNIVERSO_CVLABEL_NEGOCIOSA B
  ON A.RFC = B.RFC AND A.FECHA = B.FECHA
LEFT JOIN RSS_MCV_UNIVERSO_DALABEL_NEGOCIOSA C
  ON A.RFC = C.RFC AND A.FECHA = C.FECHA
LEFT JOIN dbriskdatamart.MZM_MCV_UNIVERSO_MODELADO_201908 D
