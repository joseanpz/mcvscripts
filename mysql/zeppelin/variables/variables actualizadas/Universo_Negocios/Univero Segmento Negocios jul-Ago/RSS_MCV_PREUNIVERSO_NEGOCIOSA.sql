%hive
DROP TABLE IF EXISTS RSS_MCV_PREUNIVERSO_NEGOCIOSA;

CREATE TABLE IF NOT EXISTS RSS_MCV_PREUNIVERSO_NEGOCIOSA 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' 

LOCATION 's3://boi-banregio/datalake/data/InteligenciaRiesgos/M&M/MCV/RSS/RSS_MCV_PREUNIVERSO_NEGOCIOSA' AS
...
INNER JOIN RSS_MCV_UNIVERSO_LLAVES_NEGOCIOSA B
ON A.LLAVE_UNIVERSAL_20 = B.llave_universal AND A.fecha = B.fecha
/*Para llaves universales unicas */
LEFT JOIN (SELECT distinct llave_universal from RSS_MCV_CATALOGO_LIN_REESTA) C
ON A.LLAVE_UNIVERSAL_20 = C.llave_universal 

LEFT JOIN (SELECT  distinct LLAVE_UNIVERSAL_20 from RSS_MCV_CATALOGO_LIN_NAC_VENC_NEGOCIOSA ) D
