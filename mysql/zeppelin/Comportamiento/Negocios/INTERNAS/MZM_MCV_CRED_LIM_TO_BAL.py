...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------ MZM_MCV_CRED_LIM_TO_BAL ----------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

DROP TABLE IF EXISTS dbriskdatamart.MZM_MCV_D_CRED_LIM_TO_BAL__${FECHA_CALIFICACION};
CREATE TABLE dbriskdatamart.MZM_MCV_D_CRED_LIM_TO_BAL__${FECHA_CALIFICACION}
ROW FORMAT DELIMITED FIELDS TERMINATED BY '|'

LOCATION 's3://boi-banregio/datalake/data/InteligenciaRiesgos/M&M/MZM/MCV/MZM_MCV_D_CRED_LIM_TO_BAL__${FECHA_CALIFICACION}' AS 
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, (T2.MONTO_LINEA - T2.SALDO) AS D_CRED_LIM_TO_BAL
		...
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			...
			AND T1.RFC = T2.RFC) T2
    ON T1.RFC=T2.RFC AND T1.FECHA=T2.FECHA
    
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MAX(T2.MONTO_LINEA - T2.SALDO) AS MAX_D_CRED_LIM_TO_BAL_03M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MIN(T2.MONTO_LINEA - T2.SALDO) AS MIN_D_CRED_LIM_TO_BAL_03M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
			WHERE  T2.FECHA >= T1.FECHA_TOPE_03M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, AVG(T2.MONTO_LINEA - T2.SALDO) AS AVG_D_CRED_LIM_TO_BAL_03M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
			WHERE  T2.FECHA >= T1.FECHA_TOPE_03M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN 
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MENOS_D_CRED_LIM_TO_BAL_03M

    FROM 


        (SELECT T1.RFC, T1.FECHA, SUM((T2.MONTO_LINEA - T2.SALDO) * ((FLOOR(T1.FECHA/100)-FLOOR(T2.FECHA/100))*12+MOD(T1.FECHA,100)-MOD(T2.FECHA,100)+1)) AS PND_1
        										FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
        										LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) T2
        										ON T1.RFC = T2.RFC--  AND T1.FECHA = T2.FECHA
        									WHERE T2.FECHA >= T1.FECHA_TOPE_03M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MAS_D_CRED_LIM_TO_BAL_03M

    FROM 


        (SELECT T1.RFC, T1.FECHA, SUM((T2.MONTO_LINEA - SALDO) * (4-((FLOOR(T1.FECHA/100)-FLOOR(T2.FECHA/100))*12+MOD(T1.FECHA,100)-MOD(T2.FECHA,100)))) AS PND_1
        										FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
        										LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) T2
        										ON T1.RFC = T2.RFC --  AND T1.FECHA = T2.FECHA
        									WHERE T2.FECHA >= T1.FECHA_TOPE_03M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MAX(T2.MONTO_LINEA - T2.SALDO) AS MAX_D_CRED_LIM_TO_BAL_06M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC AND T1.FECHA=T2.FECHA
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MIN(T2.MONTO_LINEA - T2.SALDO) AS MIN_D_CRED_LIM_TO_BAL_06M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, AVG(T2.MONTO_LINEA - T2.SALDO) AS AVG_D_CRED_LIM_TO_BAL_06M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
			WHERE  T2.FECHA >= T1.FECHA_TOPE_06M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN 
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MENOS_D_CRED_LIM_TO_BAL_06M
...
LEFT JOIN
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MAS_D_CRED_LIM_TO_BAL_06M

    FROM 


        (SELECT T1.RFC, T1.FECHA, SUM((T2.MONTO_LINEA - SALDO) * (7-((FLOOR(T1.FECHA/100)-FLOOR(T2.FECHA/100))*12+MOD(T1.FECHA,100)-MOD(T2.FECHA,100)))) AS PND_1
        										FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
        										LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) T2
        										ON T1.RFC = T2.RFC--  AND T1.FECHA = T2.FECHA
        									WHERE T2.FECHA >= T1.FECHA_TOPE_06M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MAX(T2.MONTO_LINEA - T2.SALDO) AS MAX_D_CRED_LIM_TO_BAL_12M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC AND T1.FECHA =T2.FECHA
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, MIN(T2.MONTO_LINEA - T2.SALDO) AS MIN_D_CRED_LIM_TO_BAL_12M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
...
LEFT JOIN
    (SELECT T1.RFC,T1.FECHA, AVG(T2.MONTO_LINEA - T2.SALDO) AS AVG_D_CRED_LIM_TO_BAL_12M
		FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
		LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) AS T2
			ON T1.RFC = T2.RFC
			WHERE  T2.FECHA >= T1.FECHA_TOPE_12M AND T2.FECHA <= T1.FECHA
...
LEFT JOIN 
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MENOS_D_CRED_LIM_TO_BAL_12M
...
LEFT JOIN
    (SELECT T1.RFC, T1.FECHA , PND_1/PND_2 AS AVG_PND_MAS_D_CRED_LIM_TO_BAL_12M

    FROM 


        (SELECT T1.RFC, T1.FECHA, SUM((T2.MONTO_LINEA - SALDO) * (13-((FLOOR(T1.FECHA/100)-FLOOR(T2.FECHA/100))*12+MOD(T1.FECHA,100)-MOD(T2.FECHA,100)))) AS PND_1
        										FROM dbriskdatamart.MZM_MCV_FECHAS__${FECHA_CALIFICACION} T1
        										LEFT JOIN (SELECT RFC,FECHA,SUM(SALDO) AS SALDO, MAX(MONTO_LINEA) AS MONTO_LINEA FROM dbriskdatamart.TBL_CARTERACREDITICIA GROUP BY RFC,FECHA) T2
        										ON T1.RFC = T2.RFC -- AND  T1.FECHA = T2.FECHA
        									WHERE T2.FECHA >= T1.FECHA_TOPE_12M AND T2.FECHA <= T1.FECHA
 ...
 SELECT * FROM dbriskdatamart.MZM_MCV_D_CRED_LIM_TO_BAL__${FECHA_CALIFICACION} LIMIT 100;
