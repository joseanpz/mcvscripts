...
$BODY$
DECLARE
	initial_month_dc integer;
	final_month_dc integer;
BEGIN
initial_month_dc := FLOOR(initial_month/100)*12 + MOD(initial_month, 100);
final_month_dc := FLOOR(final_month/100)*12 + MOD(final_month, 100);
RETURN QUERY SELECT
	T1."LLAVE_UNIVERSAL",
	T1."FECHA",
	T1."SALDO",
	...
	CASE
		...
		WHEN T2."MESESDC" - T1."MESESDC" <> months THEN NULL::VARCHAR
...
WHERE
	...
	AND T2."MESESDC" BETWEEN  initial_month_dc AND final_month_dc + 1 
	...
	AND T2."MESESDC" - T1."MESESDC" BETWEEN 0 AND months
...
FROM
	cartera."JAE_FUNCTION_MCV_ROLL_RATE_CLASIFICACION_"( MONTHS, initial_month, final_month ) T
