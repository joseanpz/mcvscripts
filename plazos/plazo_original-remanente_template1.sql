...
FROM
	...
	FROM
		(SELECT LINEA,CREDITO, MONEDA, LLAVE_UNIVERSAL, CLASIFICACION 
			FROM JAT_BKT_CARTERA_CREDITICIA_201201_201804 
			WHERE FECHA = '201709' 
			AND CLASIFICACION IN ('Actividad Comercial y Empresarial','Entidades Financieras','Entidades Gubernamentales','COMERCIAL Y EMPRESARIAL')) T1
	LEFT JOIN
		(
		SELECT
			DISTINCT
